...
const TabbedWasherDashboard = () => {
  ...
  const fetchDashboardStats = async () => {
    try {
      const response = await axios.get(`https://zuci-sbackend-8.onrender.com/api/washer/${user?.id}/dashboard`);
  ...
  const handleUpdateWash = async (leadId: number) => {
    ...
    try {
      await axios.put(`https://zuci-sbackend-8.onrender.com/api/washer/${user?.id}/update-wash/${leadId}`, {
        washStatus,
