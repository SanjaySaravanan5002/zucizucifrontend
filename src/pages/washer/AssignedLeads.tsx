...
const AssignedLeads = () => {
  ...
  const handleUpdateWash = async (leadId: number, washId: string) => {
    try {
      ...
      await axios.put(`https://zuci-backend-my3h.onrender.com/api/leads/${leadId}/wash-history/${washId}`, requestData, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const handleMarkWashCompleted = async (leadId: number, washId: string) => {
    try {
      ...
      const response = await axios.put(`https://zuci-backend-my3h.onrender.com/api/leads/${leadId}/monthly-subscription/wash/${washId}`, requestData, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const handleUpdateOneTimeWash = async (leadId: number) => {
    try {
      ...
      const response = await axios.put(`https://zuci-backend-my3h.onrender.com/api/leads/${leadId}/onetime-wash/update`, {
        status: 'completed',
        feedback: updateData.feedback || '',
  ...
  const handleOpenMap = async (lead: AssignedLead) => {
    try {
      ...
      const token = localStorage.getItem('auth_token');
      const response = await axios.get(`https://zuci-backend-my3h.onrender.com/api/leads/${lead.id}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
